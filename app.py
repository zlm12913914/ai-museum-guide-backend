...
import websockets # For AssemblyAI
import sqlite3
...
# OpenAI配置
client = OpenAI(
    api_key=os.environ.get("OPENAI_API_KEY") # 从环境变量加载
)
...
# --- 新增：WebSocket 实时转录处理 ---
@sockets.route('/ws/transcribe')
def transcribe_socket(ws):
    """处理来自客户端的音频流，并转发给 AssemblyAI"""
    if not ASSEMBLYAI_API_KEY:
        logger.error("AssemblyAI API Key未配置")
        ws.close(code=1008, reason="ASR service not configured")
        return
    ...
    async def forward_to_assemblyai():
        try:
            ...
            async with websockets.connect(
                ...
                logger.info("成功连接到 AssemblyAI")

                # 2. 定义两个协程：一个从客户端接收并发送给AAI，一个从AAI接收并发送给客户端
                async def send_to_aai(client_ws):
                    try:
                        while not client_ws.closed:
                            message = client_ws.receive()
                            if message:
                                # 将从客户端收到的音频数据（二进制）编码为 JSON 发送给 AAI
                                data = {"audio_data": base64.b64encode(message).decode("utf-8")}
                                await assemblyai_ws.send(json.dumps(data))
                            else:
                                break
                    except Exception as e:
                        logger.warning(f"客户端发送循环中断: {e}")
                    # 发送终止消息
                    await assemblyai_ws.send(json.dumps({"terminate_session": True}))

                async def receive_from_aai(client_ws):
                    try:
                        async for message in assemblyai_ws:
                            ...
                            if transcript.get('message_type') == 'FinalTranscript' and transcript.get('text'):
                                logger.info(f"收到最终文本: {transcript['text']}")
                                # 将最终文本发送回客户端
                                if not client_ws.closed:
                                    client_ws.send(json.dumps({"status": "success", "transcript": transcript['text']}))
                    except Exception as e:
                        logger.error(f"从 AssemblyAI 接收时出错: {e}")
                        if not client_ws.closed:
                            client_ws.send(json.dumps({"status": "error", "message": "ASR failed"}))

                # 3. 并发运行这两个任务
                sender_task = asyncio.create_task(send_to_aai(ws))
                receiver_task = asyncio.create_task(receive_from_aai(ws))
                await asyncio.gather(sender_task, receiver_task)

        except Exception as e:
            logger.error(f"AssemblyAI WebSocket 协程出错: {e}")
            if not ws.closed:
                ws.send(json.dumps({"status": "error", "message": "Could not connect to ASR service"}))
        finally:
            if not ws.closed:
                ws.close()
            logger.info("AssemblyAI WebSocket 会话结束")

    # 运行异步的转发函数
    asyncio.run(forward_to_assemblyai())


# --- 以下是你的原始 HTTP API 端点，保持不变 ---
# ... (你的 / , /api/generate_route, /api/chat 等路由代码保持不变) ...
...
def index():
    return jsonify({
        ...
        "version": "2.0.0",
        "status": "运行中",
        "features": ["OpenAI TTS", "智能缓存", "个性化语音", "AssemblyAI ASR"] # 更新特性列表
    })
...
# ... (你其他的API路由，如/api/audio, /api/navigation等，保持不变) ...
...
if __name__ == '__main__':
    ...
    print("服务器正在启动，访问 http://127.0.0.1:5000")
